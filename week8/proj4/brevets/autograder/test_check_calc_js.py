...
def file():
    ...
    vocab_html = os.path.join(templates_dir, "calc.js")
...
def test_check_uses_getJSON(file):
    ...
    assert has_getjson, "No jQuery getJSON function found in calc.js"


def test_check_uses_jquery(file):
    jquery_pattern = r"\$\(.*\)"
    has_jquery = re.findall(jquery_pattern, file, re.MULTILINE)
    assert has_jquery, "No jQuery found in calc.js"
